...
void sendData(String command, int timeout, boolean debug) {
  ...
  if (debug) {
    Serial.println("Error: Could not send command");
  }
}
...
void loop()
{
  ...
  if(esp8266.available()) // check if the esp is sending a message 
  {
    if(esp8266.find("+IPD,"))
    {
     ...
     webpage += "<meta name=\"viewport\" content=\"width=device-width, initial-scale=1.0\"><style>.button {padding: 15px 32px;font-size: 16px;border-radius: 12px; text-align: center;display: inline-block;font-size: 16px;} .centre {text-align: center;}";
     webpage += ".green {background-color: #4CAF50;color: white;}";
     webpage += ".red {background-color: #f44336;color: white;}";
     webpage += "</style></head><body class=\"centre\"><h1 class=\"centre\">ESP8266 WEB SWITCH</h1>";
     ...
     webpage += "<button class=\"button green\" href=\"?on\">Turn on LED</button><br><br>";
     webpage += "<button class=\"button red\" href=\"?off\">Turn off LED</button></body></html>";
